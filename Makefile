...
# paas.il/privileged-kata-containers-webhook-bundle:$VERSION and paas.il/privileged-kata-containers-webhook-catalog:$VERSION.
IMAGE_TAG_BASE ?= paas.il/privileged-kata-containers-webhook
...
O = MyCompany
OU = MyDivision
CN = privileged-kata-containers-webhook-service.$(NAMESPACE).svc
...
[alt_names]
DNS.1 = privileged-kata-containers-webhook-service.$(NAMESPACE).svc
endef
...
# Generate yaml bundle for the controller
yaml-bundle: 
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	mkdir -p deploy
